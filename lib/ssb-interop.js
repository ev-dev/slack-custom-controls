...
window.initCustomControls = () => {
  // Interval to persist controls after navigating to new Workspace
	window.customControlsInterval = setInterval(() => {
...
window.toggleControls = () => {
	if (window.controlsEnabled) {
		clearInterval(window.customControlsInterval)
		$('.custom-controls-div').remove()
...
document.addEventListener('DOMContentLoaded', () => {  
  ...
  document.addEventListener('keydown', ({ keyCode, metaKey }) => {
    ...
    if (keyCode === 71 && metaKey) window.toggleControls()
  })
})
